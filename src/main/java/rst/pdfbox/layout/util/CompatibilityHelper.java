...
public class CompatibilityHelper {
	...
	/**
	 * Returns the bullet character for the given level. Actually only two bullets
	 * are used for odd and even levels. For odd levels the {@link #BULLET bullet}
	 * character is used, for even it is the {@link #DOUBLE_ANGLE double angle}. You
	 * may customize this by setting the system properties
	 * <code>pdfbox.layout.bullet.odd</code> and/or
	...
	public static void transform(final PDPageContentStream contentStream, float a, float b, float c, float d, float e,
			float f) throws IOException {
		contentStream.transform(new Matrix(a, b, c, d, e, f));
	}

	public static void curveTo(final PDPageContentStream contentStream, float x1, float y1, float x2, float y2,
	...
	public static PDPageContentStream createAppendablePDPageContentStream(final PDDocument pdDocument,
			final PDPage page) throws IOException {
		return new PDPageContentStream(pdDocument, page, AppendMode.APPEND, true, true);
	}

	public static void drawImage(final BufferedImage image, final PDDocument document,
			final PDPageContentStream contentStream, Position upperLeft, final float width, final float height)
			throws IOException {
	...
	public static PDAnnotationLink createLink(PDPage page, PDRectangle rect, Color color, LinkStyle linkStyle,
			final String uri) {
	...
	public static PDRectangle transformToPageRotation(final PDRectangle rect, final PDPage page) {
		...
		float[] points = new float[] { rect.getLowerLeftX(), rect.getLowerLeftY(), rect.getUpperRightX(),
				rect.getUpperRightY() };
