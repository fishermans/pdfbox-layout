...
public class VerticalLayout implements Layout {
	 ...
	 * 
	 * @return <code>true</code> if empty space (e.g. empty lines) should be removed
	 *         at the begin of a page.
	 */
	...
	/**
	 * Indicates if empty space (e.g. empty lines) should be removed at the begin of
	 * a page. See {@link Drawable#removeLeadingEmptyVerticalSpace()}
	 * 
	...
	public void render(final RenderContext renderContext, Drawable drawable, final LayoutHint layoutHint)
			throws IOException {
	 ...
	 * @throws IOException by pdfbox
	 */
	protected void renderAbsolute(final RenderContext renderContext, Drawable drawable, final LayoutHint layoutHint,
	...
	/**
	 * Renders the drawable at the {@link RenderContext#getCurrentPosition() current
	 * position}. This method is responsible taking any top or bottom margin
	 * described by the (Vertical-)LayoutHint into account. The actual rendering of
	 * the drawable is performed by
	 ...
	 * @throws IOException by pdfbox
	 */
	protected void renderReleative(final RenderContext renderContext, Drawable drawable, final LayoutHint layoutHint)
			throws IOException {
		...
		if (layoutHint instanceof VerticalLayoutHint) {
			...
			if (verticalLayoutHint.getMarginTop() > 0) {
				layoutAndDrawReleative(renderContext, new VerticalSpacer(verticalLayoutHint.getMarginTop()),
						verticalLayoutHint);
			}
		}
		...
		if (verticalLayoutHint != null) {
			if (verticalLayoutHint.getMarginBottom() > 0) {
				layoutAndDrawReleative(renderContext, new VerticalSpacer(verticalLayoutHint.getMarginBottom()),
						verticalLayoutHint);
			}
		}
	}
	 ...
	 * @throws IOException by pdfbox
	 */
	protected void layoutAndDrawReleative(final RenderContext renderContext, Drawable drawable,
		...
		while (renderContext.getRemainingHeight() < drawablePart.getHeight()) {
			...
			drawReletivePartAndMovePosition(renderContext, divided.getFirst(), layoutHint, true);
	 ...
	 * <code>movePosition</code> - moves to the new Y position. Any left or right
	 * margin is taken into account to calculate the position and alignment.
	 * 
	 * @param renderContext the context providing all rendering state.
	 * @param drawable      the drawable to draw.
	 * @param layoutHint    the layout hint used to layout.
	 * @param movePosition  indicates if the position should be moved (vertically)
	 *                      after drawing.
	 * @throws IOException by pdfbox
	 */
	protected void drawReletivePartAndMovePosition(final RenderContext renderContext, Drawable drawable,
			final LayoutHint layoutHint, final boolean movePosition) throws IOException {
		...
		drawable.draw(renderContext.getPdDocument(), contentStream, renderContext.getCurrentPosition().add(offsetX, 0),
				renderContext);
