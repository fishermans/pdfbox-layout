...
public class Frame implements Element, Drawable, WidthRespecting, Dividable {
	...
	/**
	 * Creates a frame containing the inner element, optionally constraint by the
	 * given dimensions. These contraints target the border-box of the frame, means:
	...
	/**
	 * Creates a frame constraint by the given dimensions. These contraints target
	 * the border-box of the frame, means: the inner element plus padding plus
	...
	/**
	 * @return if a {@link #getShape() shape}, a {@link #getBorderStroke() stroke}
	 *         and {@link #getBorderColor() color} is set.
	 */
	...
	/**
	 * Propagates the max width to the inner item if there is a given size, but no
	 * absolute position.
	 * 
	...
	public void draw(PDDocument pdDocument, PDPageContentStream contentStream, Position upperLeft,
		...
		if (getShape() != null) {
			...
			if (getBackgroundColor() != null) {
				getShape().fill(pdDocument, contentStream, upperLeft, shapeWidth, shapeHeight, getBackgroundColor(),
						drawListener);
			}
			if (hasBorder()) {
				getShape().draw(pdDocument, contentStream, upperLeft, shapeWidth, shapeHeight, getBorderColor(),
