...
public class Document implements RenderListener {
	...
	/**
	 * Creates a Document in A4 with orientation portrait and the given margins. By
	 * default, a {@link VerticalLayout} is used.
	 * 
	...
	/**
	 * Adds a (custom) {@link Renderer} that may handle the rendering of an element.
	 * All renderers will be asked to render the current element in the order they
	 * have been added. If no renderer is capable, the default renderer will be
	 * asked.
	 * 
		...
		for (Entry<Element, LayoutHint> entry : elements) {
			...
			if (!success) {
				throw new IllegalArgumentException(
						String.format("neither layout %s nor the render context knows what to do with %s",
	...
	/**
	 * Adds a {@link RenderListener} that will be notified during {@link #render()
	 * rendering}.
	 * 
