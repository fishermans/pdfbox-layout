...
public class RoundRect extends AbstractShape {
	...
	/**
	 * Creates a rounded rect with equal radiuss for both x-axis and y-axis (quarter
	 * of a circle).
	 * 
	...
	/**
	 * Creates a rounded rect with potentially different radiuss for both x-axis and
	 * y-axis (quarter of an ellipse).
	 * 
	...
	@Override
	public void add(PDDocument pdDocument, PDPageContentStream contentStream, Position upperLeft, float width,
	...
	protected void addRoundRect(PDPageContentStream contentStream, Position upperLeft, float width, float height,
		...
		CompatibilityHelper.curveTo(contentStream, b.getX() + bezX, b.getY(), c.getX(), c.getY() + bezY, c.getX(),
				c.getY());
		// contentStream.addLine(c.getX(), c.getY(), d.getX(), d.getY());
		addLine(contentStream, c.getX(), c.getY(), d.getX(), d.getY());
		CompatibilityHelper.curveTo(contentStream, d.getX(), d.getY() - bezY, e.getX() + bezX, e.getY(), e.getX(),
				e.getY());
		// contentStream.addLine(e.getX(), e.getY(), f.getX(), f.getY());
		addLine(contentStream, e.getX(), e.getY(), f.getX(), f.getY());
		CompatibilityHelper.curveTo(contentStream, f.getX() - bezX, f.getY(), g.getX(), g.getY() - bezY, g.getX(),
				g.getY());
		addLine(contentStream, g.getX(), g.getY(), h.getX(), h.getY());
		CompatibilityHelper.curveTo(contentStream, h.getX(), h.getY() + bezY, a.getX() - bezX, a.getY(), a.getX(),
				a.getY());
	}

	/**
	 * Using lines won't give us a continuing path, which looks silly on fill. So we
	 * are approximating lines with bezier curves... is there no better way?
	 */
