...
public class TextFlowUtil {
	 ...
	 */
	public static TextFlow createTextFlow(final String text, final float fontSize, final PDFont font)
			throws IOException {
	...
	public static TextFlow createTextFlowFromMarkup(final String markup, final float fontSize, final BaseFont baseFont)
			throws IOException {
		return createTextFlowFromMarkup(markup, fontSize, baseFont.getPlainFont(), baseFont.getBoldFont(),
				baseFont.getItalicFont(), baseFont.getBoldItalicFont());
	}

	/**
	 * Creates a text flow from the given text. The text may contain line breaks,
	 * and also supports some markup for creating bold and italic fonts. The
	 * following raw text
	 * 
	 * <pre>
	 ...
	 * @throws IOException by pdfbox
	 */
	public static TextFlow createTextFlowFromMarkup(final String markup, final float fontSize, final PDFont plainFont,
			final PDFont boldFont, final PDFont italicFont, final PDFont boldItalicFont) throws IOException {
	 ...
	 * @throws IOException by pdfbox
	 */
	protected static TextFlow createTextFlow(final Iterable<CharSequence> parts, final float fontSize,
			final PDFont plainFont, final PDFont boldFont, final PDFont italicFont, final PDFont boldItalicFont)
			throws IOException {
			...
			if (fragment instanceof ControlCharacter) {
				...
				if (fragment instanceof IndentCharacter) {
					...
					if (currentIndent.getLevel() == 0) {
						...
						while (!indentStack.isEmpty() && indentStack.peek() != null
				...
				if (annotationMap.isEmpty()) {
					StyledText styledText = new StyledText(fragment.toString(), currentFontSize, font, color,
							baselineOffset);
					result.add(styledText);
				} else {
					AnnotatedStyledText styledText = new AnnotatedStyledText(fragment.toString(), currentFontSize, font,
							color, baselineOffset, annotationMap.values());
					result.add(styledText);
				}
			}
		}
	...
	protected static PDFont getFont(boolean bold, boolean italic, final PDFont plainFont, final PDFont boldFont,
	...
	/**
	 * Creates a char sequence where new-line, asterisk and underscore are replaced
	 * by their corresponding {@link ControlCharacter}.
	 * 
	...
	/**
	 * Splits the sequence by the given control character and replaces its markup
	 * representation by the {@link ControlCharacter}.
	 * 
	 ...
	 */
	protected static Iterable<CharSequence> splitByControlCharacter(ControlCharacterFactory controlCharacterFactory,
