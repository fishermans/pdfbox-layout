...
public class IndentCharacters {
	...
	public static class IndentCharacter extends ControlCharacter {
		...
		/**
		 * @return the next label to use on a subsequent indent. Makes only sense for
		 *         enumerating indents.
		 */
		...
		public Indent createNewIndent(final float fontSize, final PDFont font, final Color color) throws IOException {
			return new Indent(nextLabel(), level * indentWidth, indentUnit, fontSize, font, Alignment.Right, color);
		}
	 ...
	 * character and the brackets are optional. Default indentation is 4 characters,
	 * default unit is <code>em</code> and the default bullet depends on
	 * {@link CompatibilityHelper#getBulletCharacter(int)}. It can be escaped with a
	 * backslash ('\').
	 */
	 ...
	 * characters in markup, using <code>a)...b)...etc</code> as the enumeration.
	 * The number, the unit, enumeration type/separator, and the brackets are
	 * optional. Default indentation is 4 characters, default unit is
	 ...
	 * enumerators see {@link EnumeratorFactory}.It can be escaped with a backslash
	 * ('\').
	 */
	public static class EnumerationCharacter extends IndentCharacter {
		...
		protected EnumerationCharacter(String level, String indentWidth, String indentUnit, String enumerationType,
				String separator) {
			super(level, indentWidth, indentUnit);
		...
		public boolean equals(Object obj) {
			...
			if (enumerator == null) {
				...
				return false;
			} else if (!enumerator.getClass().equals(other.enumerator.getClass()))
				return false;
			if (separator == null) {
	...
	private static class IndentCharacterFactory implements ControlCharacterFactory {
		...
		@Override
		public ControlCharacter createControlCharacter(String text, Matcher matcher,
			...
			if ("#".equals(matcher.group(14))) {
				return new EnumerationCharacter(matcher.group(13), matcher.group(18), matcher.group(20),
