...
public class TextSequenceUtil {
	 ...
	 * @throws IOException by pdfbox
	 */
	public static Divided divide(final TextSequence text, final float maxWidth, final float maxHeight)
			throws IOException {
	...
	private static WordWrapContext wordWrap(final WordWrapContext context, final float maxWidth, final TextFlow result)
		...
		if (maxWidth > 0 && lineLength + length > maxWidth) {
			...
			Pair<TextFragment> brokenWord = breakWord(word, length, maxWidth - lineLength, maxWidth - indentation,
					breakHard);
			if (brokenWord != null) {
				// word is broken
	...
	/**
	 * De-wraps the given text, means any new lines introduced by wrapping will be
	 * removed. Also all whitespace removed by wrapping are re-introduced.
	 * 
	...
	/**
	 * Convencience function that {@link #wordWrap(TextSequence, float) word-wraps}
	 * into {@link #getLines(TextSequence)}.
	 * 
	 * @param text     the text to word-wrap.
	 * @param maxWidth the max width to fit.
	 ...
	 */
	protected static TextFragment deriveFromExisting(final TextFragment toDeriveFrom, final String text,
		...
		return new StyledText(text, toDeriveFrom.getFontDescriptor(), toDeriveFrom.getColor(), 0, leftMargin,
				rightMargin);
	}

	private static Pair<TextFragment> breakWord(TextFragment word, float wordWidth, final float remainingLineWidth,
			float maxWidth, boolean breakHard) throws IOException {
		...
		Pair<String> brokenWord = WordBreakerFactory.getWorkBreaker().breakWord(word.getText(),
				word.getFontDescriptor(), remainingLineWidth - leftMargin, breakHard);
		if (brokenWord == null) {
			return null;
		}
	 ...
	 * @param upperLeft                   the position of the start of the first
	 *                                    line.
	 ...
	 * @param maxWidth                    if &gt; 0, the text may be word-wrapped to
	 *                                    match the width.
	 * @param lineSpacing                 the line spacing factor.
	 * @param applyLineSpacingToFirstLine indicates if the line spacing should be
	 *                                    applied to the first line also. Makes
	 *                                    sense in most cases to do so.
	 * @throws IOException by pdfbox
	 */
	public static void drawText(TextSequence text, PDPageContentStream contentStream, Position upperLeft,
			DrawListener drawListener, Alignment alignment, float maxWidth, final float lineSpacing,
	 ...
	 * @throws IOException by pdfbox
	 */
	public static float getOffset(final TextSequence textLine, final float targetWidth, final Alignment alignment)
			throws IOException {
		switch (alignment) {
		case Right:
	 ...
	 * @param maxWidth     if &gt; 0, the text may be word-wrapped to match the
	 *                     width.
	 ...
	 * @throws IOException by pdfbox
	 */
	public static float getHeight(final TextSequence textSequence, final float maxWidth, final float lineSpacing,
			final boolean applyLineSpacingToFirstLine) throws IOException {
		List<TextLine> lines = wordWrapToLines(textSequence, maxWidth);
		float sum = 0;
		for (int i = 0; i < lines.size(); i++) {
