...
public class TextLine implements TextSequence {
	...
	/**
	 * @return <code>true</code> if the line contains neither styled text nor a new
	 *         line.
	 */
		...
		if (ascent == null) {
			...
			for (TextFragment fragment : this) {
				float currentAscent = fragment.getFontDescriptor().getSize()
						* fragment.getFontDescriptor().getFont().getFontDescriptor().getAscent() / 1000;
	...
	public void drawAligned(PDPageContentStream contentStream, Position upperLeft, Alignment alignment,
			float availableLineWidth, DrawListener drawListener) throws IOException {
		...
		for (StyledText styledText : styledTextList) {
			...
			if (drawListener != null) {
				...
				drawListener.drawn(styledText, new Position(x, currentUpperLeft), styledText.getWidthWithoutMargin(),
	...
	private static class TextLineIterator implements Iterator<TextFragment> {
		...
		/**
		 * Creates an iterator of the given styled texts with an optional trailing new
		 * line.
		 * 
